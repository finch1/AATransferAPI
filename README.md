...
Implementing a relationship constraint with code between the Account and Transfer Entity is not possible with this domain model. Relationships annotated in code require the child entity to be a collection in the parent entity. In this application, both are parent entities. Foreign key constraints are implemented with SQL.
...
Enums for account status are used. Problem identified: 
- if the list changes (adding/reordering) : JPA offers solutions although rather complex.
- how to expose the list to the client: A simple way to overcome this was to create routes in the AccountController for each status, then calling a common method for all types of status updates. For these routes, query parameter are used instead of request parameter to differentiate a partial update from the rest of the requests. 

Implementing GET transfers by user ID proved cumbersome to implement with code. Accounts for each user can be returned by injecting IAccountRepository in the ITransferRepository. This doesn't seem to be the ideal way to develop a robust API. 
...
HATEOAS links are not implemented for Transfer entity as was not sure what links would be relevant since most are GET methods. Yet the concept is proved in the Account entity response.

At a later development stage, it was noticed that the way the API is designed, it cannot handle enriching the transfers data as extra columns won't match the transfers entity attributes. Perhaps there is a flexible mechanism allowing variable data to be returned by the repository interface. 
